...
- **Bicameral CLS separation** — Episodic KG (fast-write spatio-temporal graph) + Semantic ML (slow-learning GCN-based pattern extractor) use different storage representations to avoid the Stability Gap; see [ARCHITECTURE.md](documenter/docs/ARCHITECTURE.md#3-the-core-component)
- **Iterative bidirectional querying** — Agent controller orchestrates a reconciliation loop between episodic and semantic layers, enabling semantic priors to refine KG search and episodic anomalies to update semantic beliefs; see [ARCHITECTURE.md](documenter/docs/ARCHITECTURE.md#5-iterative-bidirectional-querying-protocol)
- **Domain-tailored spatio-temporal objects** — First-class `CropCycle`, `DiseaseEvent`, and `TreatmentAction` nodes with typed enums, temporal coordinates, and spatial indexing for agricultural diagnostics; see [ARCHITECTURE.md](documenter/docs/ARCHITECTURE.md#6-domain-specific-considerations)
- **Ablation-ready design** — Four configurable ablation modes (`full`, `kg_only`, `no_iterate`, `generic_kg`) and 10+ single-field config variants for hypothesis testing; see [BENCHMARKS.md](documenter/docs/BENCHMARKS.md#ablation-study)
- **bf16/fp16 safe** — All layers pass numerical stability tests in float16, bfloat16, and float32; see [TRAINING.md](documenter/docs/TRAINING.md#recommended-training-recipe)
...
- [docs/ARCHITECTURE.md](documenter/docs/ARCHITECTURE.md) — Design rationale, equations, inductive biases, known limitations
- [docs/TRAINING.md](documenter/docs/TRAINING.md) — Environment setup, hyperparameters, training recipe, troubleshooting
- [docs/BENCHMARKS.md](documenter/docs/BENCHMARKS.md) — Benchmark results, ablation study, profiling, research-quality evaluation
- [docs/API.md](documenter/docs/API.md) — Module-level API reference for all public classes and functions

## Related work

This architecture builds on the Stability Gap theorem (arXiv:2601.15313) and differentiates from AOI (arXiv:2512.13956), All-Mem (arXiv:2603.19595), and Zep/Graphiti (arXiv:2501.13956) through its iterative bidirectional querying protocol and agricultural domain tailoring. See [ARCHITECTURE.md](documenter/docs/ARCHITECTURE.md#1-motivation) for full positioning.
